...
const Message = forwardRef<HTMLDivElement, IMessage>(
  ({ isSelf, userAvatarUrl, userName, text, time, className }, ref): JSX.Element => {
    return (
      <div
        ref={ref}
        className={`w-full p-2 flex ${isSelf ? 'flex-row-reverse' : 'flex-row'} gap-2 ${className}`}
      >
        {/* Avatar */}
        ...
        <div className={`flex flex-col max-w-[70%] ${isSelf ? 'items-end' : 'items-start'}`}>
          ...
          <div className={`relative group`}>
            ...
            <span className={`text-xs text-gray-500 mt-1 ${isSelf ? 'text-right' : 'text-left'}`}>
