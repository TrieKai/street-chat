...
export class WebLLMApi implements LLMApi {
  ...
  constructor(
    ...
    if (type === "serviceWorker") {
      console.log("Create ServiceWorkerMLCEngine");
      this.webLLMHandler = {
        type: "serviceWorker",
        engine: new ServiceWorkerMLCEngine(engineConfig, KEEP_ALIVE_INTERVAL),
      };
    } else {
      console.log("Create WebWorkerMLCEngine");
      this.webLLMHandler = {
        type: "webWorker",
  ...
  private async initModel(onUpdate?: (message: string, chunk: string) => void) {
    ...
    this.webLLMHandler.engine.setInitProgressCallback(
      (report: InitProgressReport) => {
        onUpdate?.(report.text, report.text);
      }
    );
    await this.webLLMHandler.engine.reload(
  ...
  abort(): void {
    this.webLLMHandler.engine?.interruptGenerate();
  }
  ...
  async chatCompletion(
    ...
    onUpdate?: (message: string, chunk: string, usage?: CompletionUsage) => void
  ) {
    const completion = await this.webLLMHandler.engine.chatCompletion({
      stream: stream,
